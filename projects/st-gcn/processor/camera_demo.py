...
def ckpt_time(ckpt=None, printf=None):
    if not ckpt:
        return time.time()
    if printf:
        print("time consumes {:.2f} second".format(time.time() - ckpt))
    return time.time() - ckpt, time.time()
...
class Demo(IO):
    ...
    def start(self):
        ...
        print('openpose command: ', command_line)
        import ipdb;ipdb.set_trace()
        ...
        ckpt, time5 = ckpt_time(time4, 1)

        ckpt, _ = ckpt_time(time1)
        print("\n\nTotal time consumes {:.2f} second".format(ckpt))
    @staticmethod
